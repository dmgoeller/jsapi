    ...
    module Methods
      ...
      [true, false].each do |bang|
        ...
        do |operation_name = nil, omit: nil, status: nil, strong: false, &block|
          ...
          operation_model = _api_operation_model(operation_name, definitions)
          response_model = _api_response_model(operation_model, status, definitions)
      ...
      def api_params(operation_name = nil, strong: false)
        definitions = api_definitions
        _api_params(
          _api_operation_model(operation_name, definitions),
          definitions,
          strong: strong
        )
      end
      ...
      def _api_operation_model(operation_name, definitions)
        operation_model = definitions.find_operation(operation_name)
      ...
      def _api_response_model(operation_model, status, definitions)
        response_model = operation_model.response(status)
        return response_model.resolve(definitions) if response_model
