  ...
  module Controller
    class ResponseTest < Minitest::Test
      ...
      def test_initialize_raises_an_error_when_omit_is_invalid
        content_model = Meta::Content.new(type: 'boolean')

        error = assert_raises(InvalidArgumentError) do
          Response.new({}, content_model, definitions, omit: :foo)
        end
      ...
      def test_to_json_on_boolean
        ...
        response = Response.new(true, content_model, definitions)
        assert_equal('true', response.to_json)

        response = Response.new(false, content_model, definitions)
        assert_equal('false', response.to_json)

        response = Response.new(nil, content_model, definitions)
        assert_equal('null', response.to_json)
      end

      def test_to_json_on_integer
        content_model = Meta::Content.new(type: 'integer')

        response = Response.new(1, content_model, definitions)
        assert_equal('1', response.to_json)

        response = Response.new(1.0, content_model, definitions)
        assert_equal('1', response.to_json)
      ...
      def test_to_json_on_integer_with_conversion
        content_model = Meta::Content.new(type: 'integer', conversion: :abs)
      ...
      def test_to_json_on_number
        content_model = Meta::Content.new(type: 'number')
      ...
      def test_to_json_on_numbers_with_conversion
        content_model = Meta::Content.new(type: 'number', conversion: :abs)
      ...
      def test_to_json_on_string
        content_model = Meta::Content.new(type: 'string')

        response = Response.new('foo', content_model, definitions)
        assert_equal('"foo"', response.to_json)

        response = Response.new('', content_model, definitions)
      ...
      def test_to_json_on_string_with_date_format
        content_model = Meta::Content.new(type: 'string', format: 'date')

        response = Response.new('2099-12-31T23:59:59+00:00', content_model, definitions)
        assert_equal('"2099-12-31"', response.to_json)
      end

      def test_to_json_on_string_with_datetime_format
        content_model = Meta::Content.new(type: 'string', format: 'date-time')

        response = Response.new('2099-12-31', content_model, definitions)
        assert_equal('"2099-12-31T00:00:00.000+00:00"', response.to_json)
      end

      def test_to_json_on_string_with_duration_format
        content_model = Meta::Content.new(type: 'string', format: 'duration')

        duration = ActiveSupport::Duration.build(86_400)
        response = Response.new(duration, content_model, definitions)
        assert_equal('"P1D"', response.to_json)
      end

      def test_to_json_on_string_with_conversion
        content_model = Meta::Content.new(type: 'string', conversion: :upcase)
      ...
      def test_to_json_on_array
        content_model = Meta::Content.new(type: 'array', items: { type: 'string' })

        response = Response.new(%w[foo bar], content_model, definitions)
      ...
      def test_to_json_raises_an_error_on_invalid_array
        content_model = Meta::Content.new(
          type: 'array',
          items: {
            type: 'string',
            existence: true
      ...
      def test_to_json_on_object
        ...
        response = Response.new({ foo: 'bar' }, content_model, definitions)
        assert_equal('{"foo":"bar"}', response.to_json)
      ...
      def test_to_json_on_object_with_polymorphism
        ...
        content_model = Meta::Content.new(schema: 'base')

        response = Response.new({ foo: 'bar' }, content_model, definitions)
        assert_equal('{"type":"foo","foo":"bar"}', response.to_json)

        response = Response.new({ type: 'bar', bar: 'foo' }, content_model, definitions)
      ...
      def test_to_json_on_object_and_omit_nil
        ...
        response = Response.new({}, content_model, definitions, omit: :nil)
        assert_equal('{"foo":null}', response.to_json)
      ...
      def test_to_json_on_object_and_omit_empty
        ...
        response = Response.new(object, content_model, definitions, omit: :empty)
        assert_equal('{"foo":""}', response.to_json)

        response = Response.new(object, content_model, definitions)
        assert_equal('{"foo":"","bar":""}', response.to_json)
      end
      ...
      def test_to_json_raises_an_error_on_invalid_object
        ...
        response = Response.new({ foo: nil }, content_model, definitions)
      ...
      def test_to_json_raises_an_error_on_invalid_nested_object
        ...
        response = Response.new({ foo: { bar: nil } }, content_model, definitions)
      ...
      def test_to_json_raises_an_error_on_invalid_nested_additional_property
        content_model = Meta::Content.new(type: 'object')
      ...
      def test_to_json_raises_an_error_on_invalid_response
        content_model = Meta::Content.new(type: 'string', existence: true)
        response = Response.new(nil, content_model, definitions)
      ...
      def test_i18n
        ...
        response = Response.new(object, content_model, definitions, locale: :en)
        ...
        response = Response.new(object, content_model, definitions, locale: :de)
